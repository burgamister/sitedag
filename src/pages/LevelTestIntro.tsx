  ...
  const sendQuickLead = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(text || t.levelTestIntro.sendError);
      }
      ...
      setSendStatus("error");
      setSendError(error instanceof Error ? error.message : t.levelTestIntro.sendError);
    } finally {
      setIsSending(false);
    }
  };
          ...
          <section className="max-h-[90svh] w-full max-w-xl overflow-y-auto rounded-t-[1.4rem] border-0 border-foreground/25 bg-background p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] shadow-2xl sm:rounded-[1.5rem] sm:border-2 sm:p-5 sm:pb-5 md:p-7">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <h2 className="font-main text-[2rem] font-bold leading-[0.9] text-foreground sm:text-[2.35rem] md:text-5xl">{t.levelTestIntro.modalTitle}</h2>
                <p className="mt-3 font-montserrat text-[0.92rem] leading-snug text-foreground/70 sm:text-sm md:text-base">
                  {t.levelTestIntro.modalDesc}
                </p>
              </div>
              <button
                type="button"
                ...
                aria-label={t.levelTestIntro.close}
              >
                x
              </button>
            </div>
            ...
            <div className="mt-5 grid gap-3 sm:mt-6">
              <input
                ...
                placeholder={t.levelTestIntro.namePlaceholder}
                aria-label={t.levelTestIntro.namePlaceholder}
              />
              <input
                value={contact}
                onChange={(event) => setContact(event.target.value)}
                className="h-12 w-full rounded-none border-2 border-foreground/30 bg-background px-3 font-montserrat text-base text-foreground outline-none transition-colors focus:border-[hsl(0_82%_18%)]"
                placeholder={t.levelTestIntro.contactPlaceholder}
                aria-label={t.levelTestIntro.contactPlaceholder}
              />
            </div>
            ...
            <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center">
              ...
              >
                {isSending ? t.levelTestIntro.submitting : t.levelTestIntro.submit}
              </button>
              {sendStatus === "success" && <p className="font-montserrat text-sm text-[hsl(71_33%_23%)]">{t.levelTestIntro.success}</p>}
              {sendStatus === "error" && <p className="font-montserrat text-sm text-[hsl(0_82%_18%)]">{t.levelTestIntro.error}: {sendError}</p>}
            </div>
          </section>
        </div>
      )}
